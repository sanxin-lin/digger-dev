...
import { supportTouch, getStyle, getRect } from '@digger/shared';
...
function removeRipple(this: RippleHTMLElement) {
  ...
  const task = () => {
    ...
    const delay: number =
      ANIMATION_DURATION - performance.now() + Number(lastRipple.dataset.createdAt);
...
function updated(el: RippleHTMLElement, binding: DirectiveBinding<RippleOptions>) {
  ...
  const diff =
    newBinding.color !== el._ripple?.color || newBinding.disabled !== el._ripple?.disabled;
