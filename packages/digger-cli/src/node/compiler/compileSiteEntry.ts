...
import { glob, isDir, outputFileSyncOnChange } from '../shared/fs.js';
...
export async function findExamples(draftMode: boolean): Promise<string[]> {
  ...
  return mergedExamples.filter(example =>
    draftMode
      ? isDraftExample(example) || !hasDraftExample(mergedExamples, example)
      : !isDraftExample(example),
  );
}

export function filterDraftDocs(docs: string[], draftMode: boolean) {
  return docs.filter(doc =>
    draftMode ? isDraftDoc(doc) || !hasDraftDoc(docs, doc) : !isDraftDoc(doc),
  );
}
...
export async function findPageLocales(): Promise<string[]> {
  const defaultLanguage = get(await getDiggerConfig(), 'defaultLanguage');
  ...
  baseLocales.forEach(locale => {
    ...
    filterMap.set(
      routePath + language,
      slash(`${SITE_PC_DIR}/pages/${routePath}/locale/${language}.ts`),
    );
  });
...
export async function buildSiteEntry(draftMode: boolean) {
  await getDiggerConfig(true);
  await Promise.all([
    buildMobileSiteRoutes(draftMode),
    buildPcSiteRoutes(draftMode),
    buildSiteSource(),
  ]);
}
